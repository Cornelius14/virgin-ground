...
const HowItWorks = () => {
  ...
  return (
    <section id="how" className="relative w-full py-24 md:py-36 px-4 md:px-6 lg:px-12 section-dark overflow-hidden">
      <div className="absolute inset-0 opacity-20 bg-gradient-to-b from-transparent via-primary/3 to-transparent"></div>
      
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center space-y-5 max-w-3xl mx-auto mb-20 md:mb-24 px-4">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-medium tracking-[-0.02em] text-foreground leading-tight">
            How it Works — Step by Step
          </h2>
        </div>

        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-12 lg:gap-20 max-w-[1200px] mx-auto">
            ...
            {steps.map((step, index) => (
              <div key={step.number}>
                <button
                  ...
                  className={`w-full text-left p-8 rounded-xl transition-all duration-300 ${
                    activeStep === index + 1
                  ...
                  <div className="flex items-start gap-6">
                    <span 
                      className={`text-2xl font-medium transition-colors ${
                        activeStep === index + 1 ? 'text-primary' : 'text-muted-foreground/25'
                      }`}
                    >
                      {step.number}
                    </span>
                    <div className="flex-1">
                      <h3 
                        className={`text-lg font-medium mb-2.5 transition-colors leading-snug tracking-tight ${
                          activeStep === index + 1 ? 'text-foreground' : 'text-muted-foreground/45'
                        }`}
                      >
                        {step.title}
                      </h3>
                      <p 
                        className={`text-sm leading-relaxed transition-colors ${
                          activeStep === index + 1 ? 'text-muted-foreground' : 'text-muted-foreground/35'
                        }`}
                      >
                        {step.description}
                      </p>
                ...
                {index < steps.length - 1 && (
                  <div className="h-px bg-border/15 my-1 ml-8"></div>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="order-2 lg:order-last w-full">
            <div 
              className="rounded-[28px] p-1 shadow-2xl w-full bg-gradient-to-br from-border/30 to-border/15"
            >
              <div 
                className="rounded-[26px] p-8 md:p-10 lg:p-12 min-h-[600px] lg:min-h-[680px] bg-eggshell w-full"
              >
