      ...
      <div className="max-w-[1160px] mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {plans.map((plan, index) => (
            <div key={index} className={`relative premium-card p-10 hover:scale-[1.01] transition-all duration-200 ${plan.popular ? 'border-2 border-primary/20' : ''}`}>
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                  <span className="px-4 py-1.5 text-xs font-semibold rounded-full bg-primary/90 text-primary-foreground shadow-md">Most popular</span>
                </div>
              )}
              <h3 className="text-2xl font-bold mb-1 text-foreground">{plan.name}</h3>
              <div className="text-3xl font-bold mb-4 text-foreground">{plan.price}</div>
              ...
              <Button className="w-full" onClick={() => setModalOpen(true)}>{plan.buttonText}</Button>
            </div>
          ))}
        </div>
      </div>
